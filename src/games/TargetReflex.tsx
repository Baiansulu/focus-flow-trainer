...
const TargetReflex = () => {
  ...
  const spawn = useCallback(() => {
    ...
    idRef.current += 1;
    setTarget({ id: idRef.current, x, y, size, bornAt: performance.now(), ttl: ttlForDifficulty(difficulty) });
  }, [difficulty]);
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      setTimeLeft(left);
      if (left <= 0) { setRunning(false); setDone(true); setTarget(null); }
    }, 100);
  ...
  useEffect(() => {
    ...
    saveSession({
      ...
      score, accuracy, avg_reaction_ms: avgReaction || 0,
      difficulty_level: difficulty, rounds: hits + misses, mistakes: misses,
    })
  ...
  const reset = () => {
    setRunning(false); setDone(false); setTarget(null);
    setScore(0); setHits(0); setMisses(0); setReactions([]); setDifficulty(3);
  ...
  return (
    <div>
      <div className="flex items-end justify-between flex-wrap gap-4 mb-6">
        <div>
          <div className="text-xs uppercase tracking-widest text-muted-foreground">Target Reflex</div>
          <h2 className="mt-1 text-2xl md:text-3xl font-semibold">Focus. Click. Adapt.</h2>
        </div>
      ...
      >
        {!running && !done && timeLeft === ROUND_DURATION && (
          <div className="absolute inset-0 grid place-items-center">
            <div className="text-center max-w-md px-6">
              ...
              <h3 className="mt-4 text-2xl font-semibold">Ready when you are</h3>
              <p className="mt-2 text-sm text-muted-foreground">Click the targets quickly and accurately. The AI tunes speed and size to your performance.</p>
            </div>
          </div>
        )}
        ...
        {done && (
          <div className="absolute inset-0 grid place-items-center bg-background/70 backdrop-blur-sm">
            <div className="text-center max-w-md px-6 animate-float-in">
              ...
              <h3 className="mt-2 text-3xl font-semibold">Nice work.</h3>
              <p className="mt-2 text-sm text-muted-foreground">Score {score} · Accuracy {accuracy}% · {avgReaction}ms · Lv {difficulty}</p>
        ...
        {target && (
          <button
            key={target.id}
            onMouseDown={(e) => { e.stopPropagation(); onHit(); }}
      ...
      <p className="mt-4 text-xs text-muted-foreground">Tip: misses on the empty area cost points. Stay deliberate — the controller rewards consistency.</p>
    </div>
  );
};
